// ============================================================
// 丸子の小家 · 网页版复刻 - 应用逻辑（框架 + 首页 + 计划）
// 完完全全复刻 wanzi-mini 小程序
// ============================================================
(function () {
  'use strict';
  ...
  // ===== 导航配置（复刻原版 13 页） =====
  const NAVS = [
    { id: 'index', name: '小家', icon: 'home' },
    { id: 'plan', name: '每日计划', icon: 'plan' },
    { id: 'beauty', name: '变瘦变美', icon: 'beauty' },
    { id: 'food', name: '美食日记', icon: 'food' },
    { id: 'knowledge', name: '知识库', icon: 'knowledge' },
    { id: 'status', name: '我的情况', icon: 'status' },
    { id: 'project', name: '我的项目', icon: 'project' },
    { id: 'asset', name: '我的资产', icon: 'asset' },
    { id: 'travel', name: '我的旅行', icon: 'travel' },
    { id: 'tips', name: '小贴士', icon: 'tips' },
    { id: 'yearly', name: '年度计划', icon: 'yearly' },
    { id: 'insights', name: 'AI 看板', icon: 'chart' },
    { id: 'settings', name: '设置', icon: 'gear' },
  ];
  const QUOTES_ZH = ['今天也要元气满满', '慢慢来，比较快', '把日子过成喜欢的样子', '每一步都算数', '温柔且坚定', '好好生活，慢慢相遇'];
  const QUOTES_EN = ['Stay sweet', 'One day at a time', 'Keep going', 'Be gentle', 'Small steps', 'Enjoy today'];
  ...
  function renderDrawer() {
    const list = $('drawerNavList');
    list.innerHTML = NAVS.map(n =>
      '<div class="drawer-nav-item ' + (curPage === n.id ? 'active' : '') + '" onclick="HomeUI.goPage(\'' + n.id + '\')">' +
      '<img class="drawer-nav-icon" src="icons/' + n.icon + '.svg">' +
      '<span class="drawer-nav-name">' + (S().navNames[n.id] || n.name) + '</span>' +
      '</div>').join('');
  }
  ...
  function goHome() { switchTab('index'); }
  ...
  function renderPage(page) {
    const body = $('pageBody');
    const quote = QUOTES_ZH[Math.floor(Math.random() * QUOTES_ZH.length)];
    const quoteEn = QUOTES_EN[Math.floor(Math.random() * QUOTES_EN.length)];
    $('navQuoteZh').textContent = quote;
    $('navQuoteEn').textContent = quoteEn;
    $('navTitle').textContent = S().deskName || '丸子の小家';
    $('drawerName').textContent = S().deskName || '丸子の小家';
    renderDrawer();
  ...
  function bindPage(page) {
    ...
    b.querySelectorAll('[data-act]').forEach(node => {
      node.addEventListener('click', e => {
        ...
        if (typeof PAGE_ACTIONS[page] === 'object' && PAGE_ACTIONS[page][act]) {
          PAGE_ACTIONS[page][act](id, val, node, e);
        } else if (PAGE_ACTIONS[act]) {
          PAGE_ACTIONS[act](id, val, node, e);
        }
      });
    });
    // input 绑定
  ...
  PAGES.index = function () {
    ...
    return '' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/plan.svg"><text>今日计划</text>' +
      '    <div class="ct-right"><span class="badge">' + doneCount + '/' + plans.length + '</span></div></div>' +
      (plans.length ? plans.slice(0, 5).map(p =>
        '<div class="item ' + (p.done ? 'done' : '') + '"><div class="chk" data-act="homeToggle" data-id="' + p.id + '">' + (p.done ? '<img class="chk-img" src="icons/check.svg">' : '') + '</div><text class="txt">' + h(p.text) + '</text></div>'
      ).join('') : '<div class="empty"><img class="empty-img" src="icons/plan.svg"><text>今天还没有计划～</text></div>') +
  ...
  PAGES.plan = function () {
    ...
    return '' +
      ...
      '  <img class="pt-icon" src="icons/plan.svg">' +
      ...
      // 添加计划
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/plan.svg"><text>添加计划</text></div>' +
      ...
      '    <div class="btn sm primary" data-act="addPlan"><img class="btn-icon" src="icons/plus.svg"><text>添加</text></div>' +
      '    <div class="btn sm lav" data-act="splitPlan"><img class="btn-icon" src="icons/sparkle.svg"><text>拆任务</text></div>' +
      '  </div>' +
      ...
      // 番茄钟
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/tomato.svg"><text>番茄钟</text>' +
      ...
      // 计划列表
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/list.svg"><text>计划列表</text>' +
      '    <div class="ct-right">' + (plans.length ? '<div class="btn sm ghost" data-act="clearDone">清除已完成</div>' : '') + '</div></div>' +
      (plans.length === 0 ? '<div class="empty"><img class="empty-img" src="icons/plan.svg"><text>还没有计划，快添加一个吧</text></div>' :
        '<div class="item-list">' + display.map(p =>
          '<div class="item ' + (p.done ? 'done' : '') + '">' +
          '<div class="chk" data-act="toggleDone" data-id="' + p.id + '">' + (p.done ? '<img class="chk-img" src="icons/check.svg">' : '') + '</div>' +
          '<text class="txt">' + h(p.text) + '</text>' +
          (p.time ? '<text class="time-tag">' + h(p.time) + '</text>' : '') +
          '<div class="icon-btn" data-act="delPlan" data-id="' + p.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div>' +
          '</div>').join('') + '</div>') +
  ...
  PAGES.beauty = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/beauty.svg">' +
  ...
  function beautyCareHtml(b, cares, daily, ds) {
    return '<div class="card">' +
      '<div class="card-title"><img class="ct-icon" src="icons/flower.svg"><text>每日护理</text>' +
      ...
      '<div class="item-list">' + cares.map(c =>
        ...
        '<div class="chk" data-act="toggleCare" data-id="' + c.id + '">' + (daily[c.id] ? '<img class="chk-img" src="icons/check.svg">' : '') + '</div>' +
        '<img class="item-icon" src="icons/' + (CARE_ICONS[c.id] || 'flower') + '.svg">' +
  ...
  function beautyWeightHtml(b, weights) {
    return '<div class="card">' +
      '<div class="card-title"><img class="ct-icon" src="icons/scale.svg"><text>体重记录</text></div>' +
  ...
  function beautyWaterHtml(b, daily, ds) {
    ...
    return '<div class="card">' +
      '<div class="card-title"><img class="ct-icon" src="icons/drop.svg"><text>喝水 ' + cups + '/' + goal + ' 杯</text></div>' +
  ...
  function beautyPeriodHtml(b) {
    ...
    return '<div class="card">' +
      '<div class="card-title"><img class="ct-icon" src="icons/calendar.svg"><text>经期记录</text>' +
  ...
  PAGES.travel = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/travel.svg">' +
      '  <div class="pt-text"><div class="pt-h2">我的旅行</div><div class="pt-desc">想去的地方，一起出发吧</div></div>' +
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/plus.svg"><text>加入旅行清单</text></div>' +
      ...
      '  <div class="row"><div class="btn primary" data-act="addTravel"><img class="btn-icon" src="icons/flag.svg"><text>加入清单</text></div></div>' +
      '</div>' +
      ...
      (travels.length ? '<div class="grid-2">' + travels.map(t =>
        ...
        '<div class="icon-btn" data-act="delTravel" data-id="' + t.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div></div></div>').join('') + '</div>'
        : '<div class="empty"><img class="empty-img" src="icons/travel.svg"><text>还没有旅行计划，快添加一个吧～</text></div>');
  };
  ...
  PAGES.yearly = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/yearly.svg">' +
      '  <div class="pt-text"><div class="pt-h2">年度计划</div><div class="pt-desc">' + year + ' 年 · 立个 Flag 冲鸭</div></div>' +
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/target.svg"><text>完成进度</text>' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/flag.svg"><text>立下 Flag</text></div>' +
      ...
      '  <div class="btn primary" data-act="addYearly"><img class="btn-icon" src="icons/check.svg"><text>立下 Flag</text></div></div>' +
      '</div>' +
      (yearlies.length ? '<div class="card">' +
        '<div class="card-title"><img class="ct-icon" src="icons/list.svg"><text>我的 Flag</text></div>' +
        '<div class="item-list">' + yearlies.map(y =>
          '<div class="item ' + (y.done ? 'done' : '') + '">' +
          '<div class="chk" data-act="toggleYearly" data-id="' + y.id + '">' + (y.done ? '<img class="chk-img" src="icons/check.svg">' : '') + '</div>' +
          '<text class="txt">' + h(y.goal) + '</text>' +
          (y.cat ? '<text class="time-tag">' + h(y.cat) + '</text>' : '') +
          '<div class="icon-btn" data-act="delYearly" data-id="' + y.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div>' +
          '</div>').join('') + '</div></div>' : '') +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/calendar.svg"><text>倒计时</text></div>' +
      ...
      (countdowns.length ? '<div class="item-list" style="margin-top:12px">' + countdowns.map(c =>
        ...
        '<div class="icon-btn" data-act="delCountdown" data-id="' + c.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div></div>').join('') + '</div>' : '') +
      '</div>';
  };
  ...
  PAGES.status = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/status.svg">' +
      ...
      (STATUS_STATE.tab === 'day' ?
        '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/heart.svg"><text>今天心情</text></div>' +
        '<div class="mood-row">' + MOODS.map(m =>
          '<div class="mood ' + (mood === m ? 'active' : '') + '" data-act="setMood" data-val="' + m + '">' + m + '</div>').join('') + '</div></div>' +
        '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/list.svg"><text>身体感受</text></div>' +
  ...
  function statusHealthHtml(s, ds) {
    ...
    return '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/chart.svg"><text>近 7 天健康</text></div>' +
  ...
  function statusProfileHtml(profile) {
    return '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/status.svg"><text>健康档案</text>' +
      ...
      (profile.length ? '<div class="item-list">' + profile.map(p =>
        ...
        '<div class="icon-btn" data-act="delProfile" data-id="' + p.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div></div>').join('') + '</div>'
        : '<div class="empty"><img class="empty-img" src="icons/status.svg"><text>还没有档案</text></div>') +
      '</div>' +
      '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/plus.svg"><text>添加档案</text></div>' +
  ...
  function statusWhisperHtml(s, ds) {
    ...
    return '<div class="card"><div class="card-title"><img class="ct-icon" src="icons/heart.svg"><text>悄悄话</text></div>' +
  ...
  PAGES.asset = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/asset.svg">' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/wallet.svg"><text>账户</text>' +
      ...
      (accounts.length ? '<div class="item-list">' + accounts.map(a =>
        ...
        '<div class="icon-btn" data-act="delAcc" data-id="' + a.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div></div>').join('') + '</div>'
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/edit.svg"><text>记一笔</text></div>' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/list.svg"><text>最近账单</text></div>' +
      (txs.length ? '<div class="item-list">' + txs.slice(0, limit).map(t =>
        ...
        '<div class="icon-btn" data-act="delTx" data-id="' + t.id + '"><img class="icon-btn-img" src="icons/trash.svg"></div></div>').join('') + '</div>'
  ...
  PAGES.settings = function () {
    ...
    return '' +
      '<div class="page-title">' +
      '  <img class="pt-icon" src="icons/gear.svg">' +
      '  <div class="pt-text"><div class="pt-h2">设置</div><div class="pt-desc">管理你的小家</div></div>' +
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/sparkle.svg"><text>AI 助手</text></div>' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/palette.svg"><text>主题</text></div>' +
      '  <div class="row">' +
      ...
      '</div>' +
      '<div class="card">' +
      '  <div class="card-title"><img class="ct-icon" src="icons/logo.svg"><text>关于</text></div>' +
  ...
  // ===== 导出 UI =====
  window.HomeUI = {
    goHome, switchTab, goPage, toggleDrawer, closeDrawer, renderPage,
    onTheme() {
      const themes = ['sakura', 'mint', 'lavender'];
      const cur = S().theme || 'sakura';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
      ...
      if (el && curPage === 'index') {
        const n = new Date();
        el.textContent = A.pad2(n.getHours()) + ':' + A.pad2(n.getMinutes()) + ':' + A.pad2(n.getSeconds());
      }
    }, 1000);
    renderPage('index');
  });
